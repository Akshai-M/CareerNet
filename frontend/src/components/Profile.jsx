    ...
    return (
        <div>
          ...
          <div className="max-w-4xl mx-auto dark:bg-gray-900 border border-gray-300 rounded-2xl my-6 p-8 shadow-lg">
            ...
            <div className="my-6">
              <h1 className="font-semibold text-xl text-gray-800 dark:text-white">Skills</h1>
              <div className="flex items-center gap-2 my-3">
                {user?.profile?.skills?.length !== 0
                  ? user?.profile?.skills.map((item, index) => (
                      <Badge key={index} className="px-4 py-1 text-sm font-medium bg-blue-200 text-blue-800 rounded-full">{item}</Badge>
                    ))
            ...
            <div className="grid w-full max-w-sm items-center gap-2 my-5">
              <Label className="text-md text-gray-800 dark:text-white font-bold">Resume</Label>
              {isResume ? (
                <a
                  target="_blank"
